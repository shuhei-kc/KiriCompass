...
```bash
python3 tools/lookup.py csa.db "lnsgkgsnl/1r5b1/... b - 1" --out report.txt
```
...
**.sfen DB** (連続自己対局)。フォルダ取り込みはファイル名から出典を判定して
公開/プライベートに自動振り分けし、floodgate自動更新は公開前例DBだけに書く。
各取り込み口には混在を防ぐガードが入っている。
