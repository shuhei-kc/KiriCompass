...
```bash
sleep $((RANDOM % 300)) && python3 tools/update_floodgate.py precedents.db
```
...
**掘り棋譜DB** (連続自己対局)。フォルダ取り込みはファイル名から出典を判定して
公開/プライベートに自動振り分けし、floodgate自動更新は公開前例DBだけに書く。
各取り込み口には混在を防ぐガードが入っている。

掘り棋譜 (連続自己対局の .sfen、通称たややん掘り) はビューアの「掘り棋譜...」
ウィンドウから**専用DB** (既定 `data/sfen.db`) に取り込む。自己対局の出現数は
