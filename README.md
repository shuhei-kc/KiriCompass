...
**floodgateの更新は2段構え。** ビューアは起動時に1回だけ新規棋譜を確認する
(既定ON。遡り日数はDB内の最新対局から自動計算し最大7日 — それを超える空きは
自動では取得せず、「DB更新...」の再走査を案内する)。「今すぐ更新」ボタンでも
任意のタイミングで取得できる。配布DBを使う人はこれで十分。

30分ごとの常時更新が必要な場合 (DB管理者・cron等のヘッドレス運用) はCLIを
スケジューラで回す。対局開始5〜10分前に実行できる毎時 :20/:50 起点を推奨。
`--jitter 300` は開始前の0〜300秒のランダムな待ちで、利用者全員が同一秒に
サーバへ集中しないための行儀。DBが無ければ空で新規作成される:
...
いずれの経路でも: 参照するのは終局棋譜の日別アーカイブのみ (ライブページは
...
- 出典フィルタの電竜戦は「TSEC以外」と「TSEC」(指定局面戦中心の企画) を
  別項目で絞り込める。なお候補手統計 (出現・勝率) は常にDB全体の合算で、
  フィルタは前例一覧にのみ効く。
