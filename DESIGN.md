...
2026-07-10 / KiriCompass 第3版 (スキーマv3・GUI・floodgate逐次更新・.sfen対応を反映)
...
**DBパスの解決規則は一元定義 (db.py resolve_db_path)。** ディレクトリ成分の
ない素の名前 (`csa.db`) は常にリポジトリ直下の `data/` に解決され、実行時のカレント
ディレクトリに意味を持たせない。CLI・GUI・USIエンジン・ランチャー生成の
どこから同じ名前を渡しても同じファイルに着地することを、接続の入口
(open_for_write / open_read_only) で保証する — 役割別DBの分離は「全員が
同じファイルを見ている」ことが前提のため。絶対パスとディレクトリ付き
相対パス (`./csa.db` 等) は明示指定としてそのまま使う。
...
WCSC過去大会の既知の注意点 (やねうらお氏の調査より、将来のダウンローダ用):
...
**WCSCの公式配布版とライブ観戦版は1局に統合。** WCSC33以降では、
公式棋譜集の `WCSC36-U7-nshogi-478shogi` 形式と、観戦ページの
日時を含む `WCSC36+U7_...+20260504160000` 形式が同じ対局を表す。
同じ大会・開始時刻差2秒以内・初期局面一致・全指し手一致の場合に限り
同一局とし、先にどちらを取り込んでも既存 `game_id` を維持したまま統合する。
ライブ版からURL用event・明示的な勝敗・終局理由・解析情報を、公式版から
表示用の対局者名を採用する。WCSC33に実在する1秒差は統合する一方、
WCSC35の同じ回戦・対局者で続けて指された別対局は、時刻と指し手が異なるため
別局として両方保持する。公式版だけではライブeventを復元できないため、
推測URLは作らない。

**buoy指定手順は擬似対局に分離。** shogi-serverのbuoy対局 (TSECの指定局面戦・
本戦の時間調整等) は、サーバーが事前に並べた手順の後から対局が始まる。この手順は
対局エンジンが選んだ手ではないため、対局本体は指定局面から索引し、共通手順は
「<大会>+<手順の内容ハッシュ>#指定局面」の擬似対局として1回だけ索引する
(.sfenの課題局面と同型。result NULL なので勝率には入らない)。手数の一次情報は
棋譜内の `'buoy game starting with N moves` コメント (全buoy棋譜に存在)、
無い場合はbuoy名のN — 両者は実ファイル10,200件で完全一致を確認済み。同じ回戦
の先後入替 (bottom/top) は同一の指定局面を戦うため、内容ハッシュのキーで
自動的に1つにまとまる (実データ621組で手順の完全一致を確認)。手順が初期局面に
戻るだけのもの (ハンドシェイク・入札) は従来通り索引除外のみで擬似対局は
作らない。指定局面ちょうどで終局した対局は終局行のみを索引する。
moves本体は全対局とも完全保持。
...
(例外: WCSC28決勝の `WCSC_F1_…`、公式棋譜集の
`WCSC36-U7-nshogi-478shogi` 形式)、電竜戦は `<接頭辞>+…+開始時刻14桁` を
要求する (詳細は ingest.py detect_source)。全1,080,482件の実eventで
従来分類との一致を検証済み。

## 実測 (このリポジトリの実データ)

floodgate 2026/07 (1,958ファイル) + WCSC35 (289) + 電竜戦dr6 (526) を取り込み。
...
```
KiriCompass/
  KiriCompass.app/      ビューアのダブルクリック起動 (macOS。Terminalを
                        開かず、3.10+のpythonを探して runtime/ に記憶)
  KiriCompass.pyw       ビューアのダブルクリック起動 (Windows。pythonw
                        関連付けでコンソール窓なし。例外はダイアログ表示)
  KiriCompass.bat       同フォールバック (Windows。コンソールは出るが
                        pythonw探索・エラー表示付き)
  kifudb/
  ...
  tools/
    ...
    update_floodgate.py    floodgate逐次更新CLI
...
**依存は標準ライブラリのみ (Python 3.10+)。** 大会ダウンローダ含め全ツールが
外部ライブラリ不要 (certifi は入っていればTLS検証に使う任意依存。HTTPは
floodgate.http_get に一元化)。当初は cshogi バックエンドを
