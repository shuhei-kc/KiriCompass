...
対局エンジンが選んだ手ではない (明らかな悪手の連続で特殊局面を作る企画もある)
ため、対局本体は指定局面から索引し、共通手順は「<大会>+<buoy名>#指定局面」の
擬似対局として1回だけ索引する (.sfenの課題局面と同型。result NULL なので勝率
にも入らない)。指定手順の手数はbuoy名の開始手数N (=N-1手) から取る — 実DBの
buoy全10,090局でアンカー不一致ゼロ、354グループの実測共通手順との矛盾ゼロを
確認済み。手順が初期局面に戻るだけのもの (ハンドシェイク・入札) は従来通り
索引除外のみで擬似対局は作らない。moves本体は全対局とも完全保持。
