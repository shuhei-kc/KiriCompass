...
2026-07-08 / KC2 第2版 (評価値・読み筋・KIF対応を追加)
...
`event` の UNIQUE 制約で別フォルダの同一棋譜も重複排除。
...
KiriCompass/
  kifudb/
    ...
    db.py        スキーマ・接続 (WAL / read-only)
    ingest.py    フォルダ走査→増分取り込み・集計更新・ログ
    query.py     sfen→候補手+前例、game_id→全記録 (URL生成含む)
  tools/
    build_db.py       DB作成・更新CLI
    lookup.py         sfen→テキストレポートCLI
    precedent_gui.py  前例ビューア (sfen貼り付け検索 + ShogiHome追従モード)
    usi_engine.py     USIダミーエンジン (ShogiHome登録用、--make-launcher対応)
```

## ShogiHome連動 (kifudb/usi.py)
...
nodes=出現局数、score cp=手番側の勝率を±1000cpに線形写像、pv=各候補の後を
最頻前例で辿った手順 (PvDepth手まで、1手ごとに前例インデックスを引くだけ
なので高速)。`go infinite` (検討モード) では `stop` まで bestmove を保留する。
MultiPV / PvDepth / DbPath / SyncFile は USI option で変更可能。
...
1. DB作成・更新GUI (tkinter, ingest.py を呼ぶだけの薄い層)
2. KI2対応 (from座標がないため擬似合法手生成が必要) と7z等アーカイブ取り込み
3. 大規模データ (実100万局) での取り込み・検索ベンチマーク
4. KiriCompass 本体の作り直し時に query.py を接続
